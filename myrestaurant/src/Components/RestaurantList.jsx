...
export default class RestaurantList extends React.Component {
  ...
  render() {
    ...
    return (
      <>
        <h2 style={{color:"red"}}> Apna Restaurant </h2>
        ...
        <div>
          {[4, 3, 2, 1, 0].map((rating) => (
            <button key={rating} onClick={() => this.handleRating(rating)} style={{backgroundColor:'green',marginLeft:"20px"}}>
        ...
        <div>
          {["asc", "desc"].map((order) => (
            <button key={order} onClick={() => this.handleSort(order)} style={{backgroundColor:'yellow',marginLeft:"20px"}}>
              {order}
            </button>
          ))}
        </div>
...
//     return (
//       <>
//         <h2 style={{color:"red"}}> Chhavi Restaurant </h2>
//         {new Array(totalPages).fill(0).map((a, i) => (
